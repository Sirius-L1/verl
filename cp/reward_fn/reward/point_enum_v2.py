...
def _accuracy_reward(answer, ground_truth):
    ...
    # 1. Find the rank 'k' of the first correct point
    ...
    # 2. Calculate reward based on the zero-centered symmetric formula
    accuracy = 0.0
...
def calculate_reward(solution_str, ground_truth, extra_info=None, fmt_ratio=1.0, acc_ratio=1.0, **kwargs):
    """
    ...
    if format_reward == 0.0:
        return {
            "score": -1.0, "format": 0.0, "accuracy": -1.0, "pred": "",
            "num_pred": 0, "has_correct": 0, "first_correct": 0,
            "only_correct": 0, "is_collinear": is_collinear
        }
    
    # If format is OK, calculate the accuracy reward.
    accuracy_reward, extracted_answer, num_pred, first_correct = _accuracy_reward(
        answer, ground_truth
    )
